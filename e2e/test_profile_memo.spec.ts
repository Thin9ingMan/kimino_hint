...
test("Profile memo field should persist and be visible only to the viewer", async ({
  page,
}) => {
  ...
  // 2.5. Verify memo field IS visible but DISABLED before friendship is established
  ...
  // Memo field should be visible before friendship
  const memoFieldBefore = page.locator(
    'textarea[placeholder*="プロフィール交換後に入力できます"]',
  );
  await expect(memoFieldBefore).toBeVisible();

  // But should be disabled
  await expect(memoFieldBefore).toBeDisabled();

  // 3. Establish friendship between User A and User B (required for memo to work)
  await page.request.post(
    `${API_BASE_URL}/api/users/${userA.userId}/friendship`,
    {
  ...
  // 12. Edit the memo
  const updatedMemoText =
    "Met at tech conference 2024. Interested in React and TypeScript.";
  ...
  console.log(
    "Profile memo test passed: memo persists and is visible only to viewer",
  );
});
