  ...
  test("Quiz choices are randomized when saved", async ({ page, request }) => {
    ...
    for (let i = 0; i < totalQuestions; i++) {
      ...
      if (choices.length > 0) {
        ...
        console.log(
          `First choice result: ${isCorrect ? "CORRECT" : isIncorrect ? "INCORRECT" : "UNKNOWN"}`,
        );

        if (isIncorrect) {
          nonFirstCorrectCount++;
          console.log(
            `✓ Question ${i + 1}: Correct answer is NOT first (randomization working!)`,
          );
        } else if (isCorrect) {
        ...
        await page.screenshot({
          path: `test-results/quiz_question_${i + 1}.png`,
        });
    ...
    console.log(
      `Questions with correct answer NOT first: ${nonFirstCorrectCount}`,
    );
    console.log(
      `Questions with correct answer first: ${totalQuestions - nonFirstCorrectCount}`,
    );
