...
test.describe("Quiz Profile Validation", () => {
  test("Button disabled when attendees missing profile or quiz", async ({
    page,
    request,
  }) => {
    ...
    await page.waitForSelector('h4:has-text("イベント情報")', {
      timeout: 10000,
    });
    ...
    await expect(
      page.locator(
        "text=全員がプロフィールとクイズを作成してからクイズに挑戦できます",
      ),
    ).toBeVisible();
    ...
    console.log(
      "✓ Quiz button correctly disabled when User B has no profile/quiz",
    );
    ...
    console.log(
      "✓ Quiz button still disabled when User B has profile but no quiz",
    );
