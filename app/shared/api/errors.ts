export type ApiErrorKind = 'network' | 'unauthorized' | 'forbidden' | 'not_found' | 'bad_request' | 'server' | 'unknown';
...
export class ApiError extends Error {
  ...
  constructor(message: string, args: { kind: ApiErrorKind; status?: number; cause?: unknown }) {
    super(message);
    this.name = 'ApiError';
    this.kind = args.kind;
...
export function toApiError(err: unknown): ApiError {
  ...
  if (status === 401) {
    return new ApiError('Authentication required', { kind: 'unauthorized', status, cause: err });
  }
  if (status === 403) {
  ...
  if (status && status >= 400 && status < 500) {
    return new ApiError('Bad request', { kind: 'bad_request', status, cause: err });
  }
