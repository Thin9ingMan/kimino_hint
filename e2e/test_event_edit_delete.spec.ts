...
test('Delete Event from Events Hub', async ({ page }) => {
  ...
  // 4. Find the event card and delete button
  const eventCard = page.locator('div').filter({ hasText: 'Event to Delete' }).first();
  await expect(eventCard).toBeVisible();
  
  // Look for delete button within or near the event card
  const deleteButton = eventCard.getByRole('button', { name: /削除|delete/i });
  await expect(deleteButton).toBeVisible();
  
  // 5. Click delete button
  await deleteButton.click();
  
  // 6. Confirm deletion (if confirmation dialog exists)
  const confirmButton = page.getByRole('button', { name: /はい|確認|削除する/ });
  if (await confirmButton.isVisible()) {
    await confirmButton.click();
  }
  
  // 7. Verify event is no longer in the list
