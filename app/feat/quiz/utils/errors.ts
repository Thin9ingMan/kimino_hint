...
export class ProfileNotFoundError extends QuizError {
  constructor(userId: number) {
    super("プロフィールが見つかりません", "PROFILE_NOT_FOUND", { userId });
  }
}
...
export class LLMGenerationError extends QuizError {
  constructor(originalError: Error) {
    super("AIによる名前生成に失敗しました", "LLM_GENERATION_FAILED", {
...
export class QuizSaveError extends QuizError {
  constructor(originalError: Error) {
    super("クイズの保存に失敗しました", "QUIZ_SAVE_FAILED", {
      originalError: originalError.message,
    });
  }
}
...
export class InvalidQuizDataError extends QuizError {
  constructor(reason: string) {
    super(`クイズデータが不正です: ${reason}`, "INVALID_QUIZ_DATA", { reason });
  }
}
...
export function handleQuizError(error: unknown): QuizError {
  ...
  if (error instanceof Error && error.message.includes("fetch")) {
    return new QuizError("ネットワークエラーが発生しました", "NETWORK_ERROR", {
  ...
  if (error && typeof error === "object" && "response" in error) {
    ...
    switch (status) {
      ...
      case 404:
        return new QuizError("データが見つかりません", "NOT_FOUND", { status });
      case 500:
        return new QuizError("サーバーエラーが発生しました", "SERVER_ERROR", {
          status,
        });
      default:
        return new QuizError("APIエラーが発生しました", "API_ERROR", {
          status,
          error: apiError,
        });
    }
