...
test.describe('Quiz Edit Screen - Button Location', () => {
  test('Button should be in bottom fixed pill with new name', async ({ page, request }) => {
    ...
    const oldButtonAtTop = topSection.locator('button:has-text("固定項目を自動埋め")');
    const isOldButtonAtTopVisible = await oldButtonAtTop.count() > 0 && await oldButtonAtTop.isVisible().catch(() => false);
    
    if (isOldButtonAtTopVisible) {
      const boundingBox = await oldButtonAtTop.boundingBox();
      console.error(`FAIL: Old button "固定項目を自動埋め" found at top of screen at position y=${boundingBox?.y}`);
    ...
    const bottomPill = page.locator('[style*="position: fixed"]').filter({ hasText: 'キャンセル' });
    ...
    const wrongAnswerInputs = page.locator('input[placeholder*="間違いの選択肢"]');
