...
function readExtensionsList() {
  ...
  if (process.env.VSCODE_EXTENSIONS?.trim()) {
    return process.env.VSCODE_EXTENSIONS.split(',').map((s) => s.trim()).filter(Boolean);
  }
...
async function confirmInstall(extensions) {
  ...
  console.log('[vscode-ext] This project can install the following VS Code extensions:');
...
try {
  ...
  if (!codeCli) {
    // eslint-disable-next-line no-console
    console.warn('[vscode-ext] VS Code CLI (code) not found in PATH; skipping extension install.');
    process.exit(0);
  }
