...
test("Edit Event Name and Description from Lobby", async ({ page }) => {
  ...
  await expect(
    page.getByText("Original Event Name", { exact: false }),
  ).toBeVisible();
  await expect(
    page.getByText("Original description", { exact: false }),
  ).toBeVisible();

  // Screenshot 1: Event lobby with edit button
  await page.screenshot({
    path: "/tmp/event_lobby_with_edit_button.png",
    fullPage: true,
  });
  ...
  await page.waitForTimeout(500);
  await page.screenshot({
    path: "/tmp/event_edit_modal_filled.png",
    fullPage: true,
  });
...
test("Delete Event from Events Hub", async ({ page }) => {
  ...
  await page.screenshot({
    path: "/tmp/events_hub_with_delete_button.png",
    fullPage: true,
  });
  ...
  await page.waitForTimeout(500);
  await page.screenshot({
    path: "/tmp/delete_confirmation_modal.png",
    fullPage: true,
  });
  ...
  await page
    .waitForSelector("text=作成したイベント", { timeout: 5000 })
    .catch(() => null);
  ...
  if (hasNoEvents) {
    ...
    const createdEventsSection = page
      .locator('h5:has-text("作成したイベント")')
      .locator("..");

    // Look for event cards in this section
    const cardsInCreatedSection = createdEventsSection.locator(
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (text?.includes("Event to Delete")) {
        throw new Error(
          "Deleted event still appears in created events section",
        );
      }
    }
  }
  ...
  await page.screenshot({
    path: "/tmp/events_hub_after_deletion.png",
    fullPage: true,
  });
