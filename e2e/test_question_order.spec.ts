...
test.describe('Question Order Verification', () => {
  test('Quiz questions appear in the correct order', async ({ page, request }) => {
    ...
    for (let i = 0; i < questionCount; i++) {
      ...
      const titleInput = card.locator('input[placeholder="例: 私の趣味は何でしょう？"]');
    ...
    for (let i = 0; i < Math.min(expectedOrder.length, questionTitles.length); i++) {
      console.log(`Checking position ${i + 1}: expected "${expectedOrder[i]}", got "${questionTitles[i]}"`);
